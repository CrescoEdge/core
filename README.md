...
Core agent services (logging control, update management), loaded above the library.
...
Provides core agent services — logging control and framework update management. It is started after `library` and before `controller`.
...
cannot start. Output: `target/core-1.3-SNAPSHOT.jar`.
...
| **[Core](https://github.com/CrescoEdge/core)** — _this repo_ | Core agent services (logging control, update management), loaded above the library. |
